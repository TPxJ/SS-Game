...
onready var rfloor = $PPlayer/CheckFloor
...
func _physics_process(delta):
	if !is_death:
		gravity_apply(delta)
		directionTo = self.global_position.direction_to(p.global_position)

		pplayer.scale.x = checkLR(directionTo)
		if checkfloor():
			print(checkLR(directionTo))
			movement(delta)
			anim()
			stop = false
		else:
...
func attack():
	if !is_atck and can_atck and !is_takehit and !spit and !stop:
		is_atck = true
		animsprite.animation = "attack"
		can_atck = false
		
func anim():
	if !is_takehit and !is_atck and !spit and !stop:
		if veloc.x == 0:
			animsprite.animation = "idle"
		elif veloc.x != 0:
